...
def get_args():
    ...
    p.add_argument("--episodes-per-epoch", type=int, default=15)  # 15 episodes per epoch
...
def main():
    ...
    net_c1 = Net(obs_shape, act_shape, hidden_sizes=args.hidden_sizes,
                 concat=True, device=args.device)
    critic1 = Critic(net_c1, device=args.device).to(args.device)
    critic1_optim = torch.optim.Adam(critic1.parameters(), lr=args.critic_lr)

    net_c2 = Net(obs_shape, act_shape, hidden_sizes=args.hidden_sizes,
                 concat=True, device=args.device)
    ...
    step_per_collect = steps_per_episode * 3 * num_envs
